...
from fast_config import UltraFastConfig
...
@st.cache_resource(show_spinner="🚀 Loading AI models to GPU (one-time setup)...", hash_funcs={Config: lambda x: "config"})
def _initialize_services():
    ...
    services = {
        ...
        'clip_embedding_service': service_manager.get_service('clip_embedding_service', ClipEmbeddingService, config),
    }
    
    # Initialize LLM service at startup to keep it always loaded in GPU
    services['llm_service'] = service_manager.get_service('llm_service', LLMService, config)
    
    # Only lazy load document analyzer (heavy TrOCR model)
    services['doc_analyzer'] = None  # 📊 TrOCR model - loads on document upload
    
    # Load vector database index (lightweight)
    services['vector_db'].load_index(config.EMBEDDING_DIMENSION, config.IMAGE_EMBEDDING_DIMENSION)
    ...
    # Log LLM readiness
    if services['llm_service'] and services['llm_service'].llm:
        logging.info("🤖 LLM loaded and ready in GPU memory")
...
class DocumentSearchInterface:
    def __init__(self):
        ...
        self.clip_embedding_service = self.services['clip_embedding_service']
        self.vector_db = self.services['vector_db']
        self.llm_service = self.services['llm_service']  # Always loaded
    ...
    def run(self):
        ...
        st.info("⚡ **AI-Ready System**: LLM loaded in GPU • Document processing ~5s • Fast AI responses")
    ...
    def _render_sidebar(self):
        ...
        st.sidebar.subheader("📁 Fast Document Upload")
        uploaded_files = st.sidebar.file_uploader(
            "Upload documents (⚡ ~5-second processing with full analysis)",
            accept_multiple_files=True,
            type=['pdf', 'docx', 'txt', 'md', 'png', 'jpg', 'jpeg', 'xlsx', 'xls', 'csv']
        )
        
        # Analysis options - re-enabled with optimizations
        st.sidebar.info("⚡ Fast mode: Full analysis with speed optimizations")
        
        # Processing mode selection
        processing_mode = st.sidebar.selectbox(
            "Processing Mode",
            ["Optimized (Fast)", "Full Analysis (Slower)"],
            index=0,
            help="Optimized: First 3 pages, 1 image/page (~5s). Full: All pages, all images (~30s+)"
        )
        
        analyze_tables = st.sidebar.checkbox("Extract tables", value=True, 
                                           help="Extract tables from PDFs and Excel files")
        analyze_images = st.sidebar.checkbox("Analyze images/charts", value=True,
                                           help="Extract and analyze images and charts")
        ...
        with col1:
            if uploaded_files and st.button("⚡ Process Documents"):
                # Determine optimized mode based on user selection
                use_optimized = processing_mode == "Optimized (Fast)"
                self._process_uploaded_files_enhanced(uploaded_files, analyze_tables, analyze_images, persist_files, force_reprocess, use_optimized)
        with col2:
    ...
    def _render_search_interface(self):
        ...
        image_query = st.file_uploader("🖼️ Search by image", type=['png', 'jpg', 'jpeg'])
        ...
        with col3:
            enable_llm = st.checkbox("Generate AI response", value=False, 
                                   help="🤖 LLM ready in GPU - fast AI responses")
        with col4:
        ...
        if enable_llm:
            st.info("💡 **LLM Ready:** GPU-accelerated AI model already loaded - responses will be fast")
        
        # Search button and results
        if st.button("Search", type="primary") and (query or image_query):
            if image_query:
                self._perform_image_search(image_query, top_k, similarity_threshold)
            elif query:
    ...
    def _perform_search(self, query: str, top_k: int, threshold: float, enable_llm: bool, prioritize_visuals: bool = False):
        ...
        try:
            ...
            if enable_llm:
                # LLM is always loaded, so just show generation message
    ...
    def _perform_image_search(self, image_query, top_k: int, threshold: float):
        """Perform image search."""
        try:
            with st.spinner("🎨 Generating image embedding..."):
                image = Image.open(image_query)
                query_embedding = self.clip_embedding_service.embed_images([image])[0]

            with st.spinner("🖼️ Searching for similar images..."):
                search_results = self.vector_db.search_images(
                    query_embedding,
                    top_k=top_k,
                    threshold=threshold
                )
            ...
            st.success(f"Found {len(search_results)} similar images.")
    ...
    def _process_uploaded_files_enhanced(self, uploaded_files, analyze_tables=True, analyze_images=True, persist_files=True, force_reprocess=False, use_optimized=True):
        ...
        try:
            ...
            processing_msg = "⚡ Fast optimized processing (~5s per document)..." if use_optimized else "🔍 Full comprehensive analysis (may take 30s+ per document)..."
            with st.spinner(processing_msg):
                ...
                for i, uploaded_file in enumerate(unique_files):
                    ...
                    analysis_result = self.doc_analyzer.analyze_document(file_path, ultra_fast_mode=False, optimized_mode=use_optimized)
    ...
    def _process_uploaded_files(self, uploaded_files):
        ...
        self._process_uploaded_files_enhanced(uploaded_files, True, True, True, False, True)
    ...
    def _process_documents(self, paths: List[str]):
        ...
        if self.vector_db.index is None or self.vector_db.image_index is None:
            self.vector_db.load_index(self.config.EMBEDDING_DIMENSION, self.config.IMAGE_EMBEDDING_DIMENSION)
    ...
    def _clear_database(self):
        """Clear the vector database and document tracking"""
        try:
            # Clear vector database
            service_manager.clear_service('vector_database')
            self.vector_db = service_manager.get_service('vector_database', VectorDatabase, self.config)
            self.vector_db.load_index(self.config.EMBEDDING_DIMENSION, self.config.IMAGE_EMBEDDING_DIMENSION)
            
            # Clear document tracking
            document_tracker.tracking_data = {
                "documents": {},
                "file_paths": {},
                "processing_history": [],
                "stats": {
            ...
            import shutil
            processed_dir = self.config.BASE_DIR / "processed_files"
            ...
            st.success("Database and document tracking cleared successfully!")
        except Exception as e:
    ...
    def _display_image_search_results(self, results: List[Dict[str, Any]]):
        """Display image search results."""
        st.subheader("🖼️ Similar Images Found")
        for result in results:
            st.write(f"Similarity: {result['similarity_score']:.3f}")
            # We need a way to get the image from the index.
