...
class ChartAnalyzer:
    ...
    def _extract_text_from_image(self, image_path: str) -> str:
        """Extract text from image using OCR with CMYK handling"""
        try:
            # Use PIL for better OCR results with CMYK conversion
            pil_image = Image.open(image_path)
            
            # Convert CMYK and other problematic modes to RGB
            if pil_image.mode == 'CMYK':
                pil_image = pil_image.convert('RGB')
            elif pil_image.mode not in ['RGB', 'L', 'P']:
                pil_image = pil_image.convert('RGB')
    ...
    def _analyze_colors(self, image: np.ndarray) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Find unique colors first to determine optimal k
            unique_colors = np.unique(data, axis=0)
            unique_count = len(unique_colors)
            
            # Adaptive k selection - can't have more clusters than unique colors
            k = min(5, unique_count, max(2, unique_count))
            
            if k < 2:
                # Handle edge case with very few colors
                return [{
                    "color": [int(c) for c in unique_colors[0]] if len(unique_colors) > 0 else [0, 0, 0],
                    "percentage": 100.0
                }]
