...
class DocumentSearchPipeline:
    ...
    def build_index(self, data_paths: List[str], force_rebuild: bool = False):
        ...
        # If forcing a rebuild, delete the existing index first
        if force_rebuild:
        ...
        if not self.vector_db.load_index(
            self.config.EMBEDDING_DIMENSION, self.config.IMAGE_EMBEDDING_DIMENSION
        ):
            # If a new index was created, it may not be optimized for size.
            # Re-create it with the correct size parameters.
            num_chunks = len(embedded_chunks)
            num_images = sum(len(doc.get('images', [])) for doc in processed_docs)
            
            logger.info(f"Creating new optimized index for {num_chunks} text chunks and {num_images} images.")
            
            self.vector_db.create_index(
                embedding_dimension=self.config.EMBEDDING_DIMENSION,
                num_docs=num_chunks
            )
            self.vector_db.create_image_index(
                embedding_dimension=self.config.IMAGE_EMBEDDING_DIMENSION,
    ...
    def _ensure_index_loaded(self) -> bool:
        ...
        try:
            if self.vector_db.index is None or self.vector_db.index.ntotal == 0:
                return self.vector_db.load_index(self.config.EMBEDDING_DIMENSION, self.config.IMAGE_EMBEDDING_DIMENSION)
            return True
        except Exception as e:
